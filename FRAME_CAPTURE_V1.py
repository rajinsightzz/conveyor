...
Marking_Image="/home/dhar-dpc/INSIGHTZZ/CODE/FrameCapture/Marking_Images/"
conveyor_status = 0
...
def take_conveyor_status():
    ...
    try:
        ...
        cur = db_con.cursor()
        query = "select * from DHAR_DB.CONVEYOR_STATUS_TABLE WHERE (ID = '1');"
        cur.execute(query)
        ...
        for i, cam in enumerate(cam_list):
            ...
            if cam_id == configHashMap.get(CONFIG_KEY_NAME.TOP_CAMERA_ID) or cam_id == configHashMap.get(CONFIG_KEY_NAME.BOTTOM_CAMERA_ID):
                ...
                if PySpin.IsAvailable(deviceThroughput) and PySpin.IsReadable(deviceThroughput):
                    device_throughput = 8312000+88000*12 #10512000 #8312000+88000*25 #*2  
                ...
                if cam_id == configHashMap.get(CONFIG_KEY_NAME.TOP_CAMERA_ID):
                    if not configure_exposure(cam, 3000.0):
                ...
                cam.AcquisitionFrameRate_set = 10
            ...
            try:
                for i, cam in enumerate(cam_list):
                    ...
                    if cam_id == configHashMap.get(CONFIG_KEY_NAME.TOP_CAMERA_ID) or cam_id == configHashMap.get(CONFIG_KEY_NAME.BOTTOM_CAMERA_ID):
                        ...
                        image_result = cam.GetNextImage(2000) #1000
...
def on_message(client, userdata, message):
    """Handles incoming MQTT messages and updates conveyorDistance."""
    ...
    try:
        ...
        if isinstance(distance_value, (int, float)) and distance_value > 0:
            ...
            # print("Con-Dis",conveyorDistance)
            logging.debug(f"Distance updated: {conveyorDistance} meters")
        else:
...
def initMqttClient():
    ...
    try:
        ...
        logging.info(f"Subscribed to Topic: {configHashMap.get(CONFIG_KEY_NAME.MQTT_TOPIC)}")
        client.loop_start()
    except Exception as e:
...
if __name__ == '__main__':
    timer_st = RepeatedTimer(1 * 5, take_conveyor_status)
    loadConfiguration()
    initializeLogger()
    updateProcessID()
    initMqttClient()  
